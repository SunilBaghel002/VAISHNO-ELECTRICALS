...
// Define public and uploads directories
...
// Verify Nodemailer transport
transporter.verify((error, success) => {
  if (error) console.error("Email transport verification failed:", error);
  else console.log("Email transport is ready to send messages");
});
  ...
  const mainContent = isAdminEmail
    ? isContactForm
      ? `
        ...
        <table border="0" cellpadding="10" style="width: 100%; background-color: #f8fafc;">
        ...
        <p><a href="mailto:${email}" style="background-color: #3b82f6; color: white; padding: 10px 20px;">Reply to Customer</a></p>
      `
      : `
        <p>A new enquiry has been received for a product. Please review the details below.</p>
        <table border="0" cellpadding="10" style="width: 100%; background-color: #f8fafc;">
          <tr><td style="font-weight: bold;">Reference ID:</td><td>${refId}</td></tr>
          <tr><td style="font-weight: bold;">Name:</td><td>${name}</td></tr>
          <tr><td style="font-weight: bold;">Email:</td><td>${email}</td></tr>
    ...
    : isContactForm
    ? `
        <p>Thank you for reaching out to Vaishno Electricals. We will respond within 24-48 hours.</p>
        <p><strong>Your Inquiry Details:</strong></p>
        <table border="0" cellpadding="10" style="width: 100%; background-color: #f8fafc;">
          <tr><td style="font-weight: bold;">Name:</td><td>${name}</td></tr>
          <tr><td style="font-weight: bold;">Email:</td><td>${email}</td></tr>
          <tr><td style="font-weight: bold;">Phone:</td><td>${phone}</td></tr>
          <tr><td style="font-weight: bold;">Reference ID:</td><td>${refId}</td></td></tr>
          <tr><td style="font-weight: bold;">Inquiry Type:</td><td>${inquiry}</td></tr>
          <tr><td style="font-weight: bold;">Message:</td><td>${message}</td></tr>
          <tr><td style="font-weight: bold;">Submitted At:</td><td>${timestamp}</td></tr>
        </table>
        <p>Contact us at <a href="mailto:info@vaishnoelectricals.com">info@vaishnoelectricals.com</a>.</p>
      `
    : `
        <p>Thank you for your interest in our products at Vaishno Electricals. We have received your enquiry about <strong>${product}</strong> and will respond within 24-48 hours.</p>
        ...
        <table border="0" cellpadding="10" style="width: 100%; background-color: #f8fafc;">
          ...
          <tr><td style="font-weight: bold;">Reference ID:</td><td>${refId}</td></tr>
          <tr><td style="font-weight: bold;">Product:</td><td>${product}</td></tr>
          <td style="font-weight: bold;">Message:</td><td>${message}</td>
          <tr><td style="font-weight: bold;">Submitted At:</td><td>${timestamp}</td></tr>
        </table>
        <p>Contact us at <a href="mailto:vaishnoelectricals2@gmail.com">info@vaishnoelectricals.com</a>.</p>
      `;
  ...
  return {
    ...
    html: `
      ...
      <body style="font-family: Arial, sans-serif; color: #333;">
        <table width="100%" style="max-width: 600px; margin: 20px auto; background-color: white; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
          <tr>
            <td style="background: linear-gradient(135deg, #3b82f6, #1e40af); padding: 20px; text-align: center;">
              <img src="${
                process.env.FRONTEND_URL
              }/images/logo.svg" alt="Vaishno Electricals Logo" style="max-width: 150px;">
            </td>
          </tr>
          <tr>
          ...
          <tr>
            <td style="background-color: #f8fafc; padding: 20px; text-align: center;">
              <p style="font-size: 14px; color: #666;">
                ...
                BATA HARDWARE ROAD, Faridabad, Haryana, 121001<br>
                Phone: +91-9873906044, +91-9810110903 | Email: <a href="mailto:vaishnoelectricals2@gmail.com">info@vaishnoelectricals.com</a><br>
...
app.post("/api/admin/authenticate", async (req, res) => {
  ...
  try {
    ...
    if (!hashedPassword) throw new Error("ADMIN_PASSWORD_HASH not set");
...
app.post("/api/admin/products", authenticateAdmin, async (req, res) => {
  try {
    ...
    if (existingProduct) {
      return res.status(400).json({ error: "Product ID exists" });
    }
